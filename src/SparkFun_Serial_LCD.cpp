...
NOTE: This library is currently hardcoded to work with Serial! This means you'll need
...
SparkFun_Serial_LCD::SparkFun_Serial_LCD() {
    //pinMode(pin, OUTPUT);
    Serial.begin(9600);
    _numlines = LCD_2LINE;
...
void SparkFun_Serial_LCD::setBrightness(int val){
    if(val >= 1 && val <= 30){
...
void SparkFun_Serial_LCD::clear(){
    command(LCD_CLEARDISPLAY);
}

// Clears a single line by writing blank spaces then returning
// cursor to beginning of line
void SparkFun_Serial_LCD::clearLine(int num){
...
void SparkFun_Serial_LCD::selectLine(int num){
    if(num > 0 && num <= _numlines){
        setCursor(num, 1);
    }
}
...
void SparkFun_Serial_LCD::home(){
    command(LCD_RETURNHOME);
}

// Saves first 2 lines of txt to splash screen memory
void SparkFun_Serial_LCD::setSplash(){
    specialCommand(LCD_SETSPLASHSCREEN);
}

// Toggles splashscreen on and off
void SparkFun_Serial_LCD::toggleSplash(){
    specialCommand(LCD_SPLASHTOGGLE);
}

//  This is for text that flows Left to Right
void SparkFun_Serial_LCD::leftToRight(){
    _displaymode |= LCD_ENTRYLEFT;
    command(LCD_ENTRYMODESET | _displaymode);
}

// This is for text that flows Right to Left
void SparkFun_Serial_LCD::rightToLeft() {
    _displaymode &= ~LCD_ENTRYLEFT;
    command(LCD_ENTRYMODESET | _displaymode);
}

// Blinking cursor on/off
void SparkFun_Serial_LCD::blink(){
    _displaycontrol |= LCD_BLINKON;
    command(LCD_DISPLAYCONTROL | _displaycontrol);
}
void SparkFun_Serial_LCD::noBlink(){
    _displaycontrol &= ~LCD_BLINKON;
    command(LCD_DISPLAYCONTROL | _displaycontrol);
}

// Underline cursor on/off
void SparkFun_Serial_LCD::cursor(){
    _displaycontrol |= LCD_CURSORON;
    command(LCD_DISPLAYCONTROL | _displaycontrol);
}
void SparkFun_Serial_LCD::noCursor(){
    _displaycontrol &= ~LCD_CURSORON;
    command(LCD_DISPLAYCONTROL | _displaycontrol);
}

// Display on/off
void SparkFun_Serial_LCD::display(){
    _displaycontrol |= LCD_DISPLAYON;
    command(LCD_DISPLAYCONTROL | _displaycontrol);
}
void SparkFun_Serial_LCD::noDisplay(){
    _displaycontrol &= ~LCD_DISPLAYON;
    command(LCD_DISPLAYCONTROL | _displaycontrol);
}

// Set cursor to specific row and col values start at 1 not 0
void SparkFun_Serial_LCD::setCursor(int row, int col){
    int row_offsets[2][4] = {
        { 0x00, 0x40, 0x10, 0x50 },
        { 0x00, 0x40, 0x14, 0x54 }
    };
...
void SparkFun_Serial_LCD::createChar(int location, uint8_t charmap[]){
    location -= 1;
    location &= 0x07;
    for (int i=0; i<8; i++){
...
// Input values start with 1
void SparkFun_Serial_LCD::printCustomChar(int num){
    Serial.write((num - 1));
}
...
void SparkFun_Serial_LCD::command(uint8_t value){
    Serial.write(0xFE);
    Serial.write(value);
    delay(5);
}
void SparkFun_Serial_LCD::specialCommand(uint8_t value){
    Serial.write(0x7C);
    Serial.write(value);
    delay(5);
}
